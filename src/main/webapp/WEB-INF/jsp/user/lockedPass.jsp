...
<div class="easyui-panel">
	    ...
	    <form id="ff" method="post" style="padding-left: 15.5%;">
	    	<table>
		    	...
		    	<tr>
		    		<td>账号状态:</td>
		    		<td><input maxlength="50" class="easyui-validatebox" type="text" id="userState" value="${user.userState}" name="userState" data-options="required:true" readonly="readonly"/></td>
		    	</tr>
		    	
		    	<tr><td>
		    		<input type="hidden" name="userId" id="userId" value="${user.userId }"/>
			        <input type="hidden" name="locked" id="locked" value="${user.locked}"/>
			    </td></tr>
...
<%-- 		//点击提交后
		...
		$(function(){
			...
			$('#okButtom').unbind('click').click(function(){
				...
				$.post('<%=basePath %>user/lockedUpdate',{userId:${user.userId},locdedId:${user.locked}})
				.done(function(data){
					if(data>0){
						$.messager.alert('提示','操作成功');
						$("#appendChild").dialog('close');
